<template>
  <div class="vl-page">
    <PageHeader title="整改任务" description="状态/负责人/逾期筛选;草稿→派发→执行→提交→验收分离,不可自验收。">
      <template #actions>
        <VlButton v-if="canAct" variant="primary" data-testid="create-task-draft" @click="createDraft">创建任务</VlButton>
      </template>
    </PageHeader>
      ...
      <table class="vl-task-table" data-testid="task-table">
        ...
        <tbody>
          <tr v-for="task in visible" :key="task.id" :data-resource-id="task.id">
            ...
            <td>{{ task.owner }}</td>
            <td class="vl-number">{{ task.due }}</td>
            <td>
              <span v-if="task.overdue" class="vl-task-table__overdue">已逾期</span>
              <StatusBadge kind="task" :state="task.status" />
            </td>
            <td><StatusBadge kind="severity" :state="task.priority" /></td>
            <td>
              <VlButton v-if="canAct" variant="ghost" size="small" data-testid="task-confirm" @click="advance(task)">
                {{ nextAction(task) }}
              </VlButton>
            </td>
          </tr>
    ...
    <p v-if="!visible.length" class="vl-task-table__empty">当前筛选下没有任务。</p>
  </div>
</template>
...
interface TaskRow { id: string; title: string; source: string; owner: string; due: string; status: TaskStatus; priority: string; overdue: boolean }

const items = ref<TaskRow[]>([
  { id: 'task-1', title: '退款率异常整改', source: '关联风险 R-204', owner: '数据团队', due: '2026-09-08', status: 'IN_PROGRESS', priority: 'HIGH', overdue: true },
  { id: 'task-2', title: '支付失败率复盘', source: '关联风险 R-302', owner: '运营团队', due: '2026-09-15', status: 'OPEN', priority: 'CRITICAL', overdue: false },
  { id: 'task-3', title: '字段治理复核', source: '关联风险 R-101', owner: '运营团队', due: '2026-09-20', status: 'PENDING_REVIEW', priority: 'MEDIUM', overdue: false },
])
...
function nextAction(task: TaskRow): string {
  switch (task.status) {
...
function advance(task: TaskRow) {
  // 演示环境仅示意推进;正式任务操作须等待服务端确认,不做误导性乐观更新
  switch (task.status) {
...
function createDraft() {
  items.value.push({ id: `task-${Date.now()}`, title: '新建整改草稿', source: '人工创建', owner: '待分配', due: '—', status: 'DRAFT', priority: 'MEDIUM', overdue: false })
}
</script>
