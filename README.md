...
### 原始文件不落盘（对计划 §4.3/§10.4 的有意偏离）

计划写的是「上传至隔离目录」、原始文件在治理成功后 24 小时内删除，并为此列了
`file_assets` 表。**本实现不落盘原始文件**：上传时在内存里解析完就丢弃字节，只保留
解析后的**脱敏行**（`datasets.governance.source`）。

这是刻意选的，不是漏做：

- **收益**：任何时刻都不存在未脱敏原文——没有需要 24 小时清理的隔离区，也没有
  「清理任务失败导致原文长期驻留」这条风险路径。计划里那套保留期与清理策略
  在本实现下是**空集**，而不是「没做」。
- **代价**：无法审计原始文件；无法在解析之后换一种编码重新解析（编码必须在上传时选对，
  选错会被 `DecodeError` 挡下，见上文）；§4.3 的「隔离目录」在本实现中不存在。

重新治理（改映射、换工作表、换时间策略）仍然可用——源行以脱敏形式暂存在
`datasets.governance.source` 里，XLSX 的多张工作表也一并暂存。真正失去的只有原始字节。

要改成按计划落盘，需要：`file_assets` 表 + 私有目录 + 24 小时清理任务 + 上传路径改写；
**并且在真实试点前先与企业确认原文保留期的合规口径**（计划 §10.4 明确要求把备份剩余
保留与第三方无法即时撤回写成合同条款）。
